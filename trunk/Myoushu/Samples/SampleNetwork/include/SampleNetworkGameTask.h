...
/** The GameTask for the SampleNetwork application of the Myoushu engine. */
class SampleNetworkGameTask : public Myoushu::GameTask
{
	public:
		/**
		 * Constructor.
		 ...
		 */
		SampleNetworkGameTask(int priority, const std::string& name, double executionInterval = 0.0, unsigned int iterationLimit = 1);

		/** Destructor */
		~SampleNetworkGameTask();
		...
		/**
		 * Process a connect request message from a host.
		 * @param networkMessage The network message.
		 */
		bool processConnect(Myoushu::NetworkMessage *networkMessage);

		/**
		 * Process a acknowledgement of connect request message from a host.
		 * @param networkMessage The network message.
		 */
		bool processAckConnect(Myoushu::NetworkMessage *networkMessage);
